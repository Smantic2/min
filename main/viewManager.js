...
function getWindowFromViewContents(webContents) {
  // Find the viewId that matches the webContents
  var viewId = Object.keys(viewMap).find(function (id) {
    return viewMap[id].webContents === webContents;
  });
  ...
  // First, try to find the window where this view is currently selected
  var selectedWindow = windows.getAll().find(function (win) {
    return windows.getState(win).selectedView === viewId;
  });
  ...
  // If not selected, return the first available window
  // (the view exists but might not be the active tab)
  return windows.getCurrent() || windows.getAll()[0] || null;
}
