...
// permissionManager is loaded before main.js in the build
// and is available globally via module.exports
...
function getWindowFromWebContents(webContents) {
  return windows.getAll().find(function (win) {
    const contents = getWindowWebContents(win);
    return contents && contents.id === webContents.id;
  });
}
