...
// Custom navbar color state
let customNavbarColor = null;
let customNavbarOpacity = 100;
...
function setColor(bg, fg, isLowContrast) {
  // Apply custom navbar color with opacity if set (custom color overrides site theme)
  let effectiveBg = bg;
  let effectiveFg = fg;
  if (customNavbarColor) {
    effectiveBg = applyOpacityToColor(customNavbarColor, customNavbarOpacity);
    // Recalculate foreground color for custom background
    const rgb = getColorFromString(customNavbarColor);
    effectiveFg = getTextColor(rgb);
  }

  document.body.style.setProperty("--theme-background-color", effectiveBg);
  document.body.style.setProperty("--theme-foreground-color", effectiveFg);
  ...
  for (let i = 0; i < backgroundElements.length; i++) {
    backgroundElements[i].style.backgroundColor = effectiveBg;
  }

  for (let i = 0; i < textElements.length; i++) {
    textElements[i].style.color = effectiveFg;
  }

  if (effectiveFg === "white") {
...
function applyOpacityToColor(hexColor, opacity) {
...
const tabColor = {
  ...
  initialize: function () {
    ...
    customNavbarColor = settings.get("navbarColor") || null;
    customNavbarOpacity =
      settings.get("navbarOpacity") !== undefined
        ? settings.get("navbarOpacity")
        : 100;
  ...
  updateColors: function () {
    ...
    if (tab.private) {
      // Temporarily disable custom color for private tabs
      const savedCustomColor = customNavbarColor;
      customNavbarColor = null;
      setColor(defaultColors.private[0], defaultColors.private[1]);
      customNavbarColor = savedCustomColor;
      return;
    }

    // If custom navbar color is set, it takes precedence over site theme
    if (customNavbarColor) {
      // setColor will apply the custom color
      if (window.isDarkMode) {
        return setColor(defaultColors.darkMode[0], defaultColors.darkMode[1]);
      }
      return setColor(defaultColors.lightMode[0], defaultColors.lightMode[1]);
